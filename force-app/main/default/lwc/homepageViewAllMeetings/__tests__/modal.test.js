...
it('owns list query then gates it in calendar mode',async()=>{const el=await mount();expect(graphql.getLastConfig().query).toContain('HomepageMeetings');[...el.shadowRoot.querySelectorAll('lightning-tab')].find(t=>t.value==='calendar').dispatchEvent(new CustomEvent('active'));await flush();expect(el.shadowRoot.querySelector('c-homepage-calendar-meetings')).not.toBeNull();expect(el.shadowRoot.querySelector('.mobile-list')).toBeNull();});
...
it('loads 1100 records through explicit continuation without false exact counts',async()=>{
 const el=await mount();let summary;el.addEventListener('summarychange',e=>{summary=e.detail;});
 ...
 expect(summary.exactCountOrNull).toBeNull();expect(graphql.getLastConfig().variables.after).toBe('e8');
 [...el.shadowRoot.querySelectorAll('lightning-button')].find(b=>b.label==='Load more meetings').click();await flush();expect(graphql.getLastConfig().variables.after).toBe('e9');
 graphql.emit(response(Array.from({length:100},(_,i)=>meeting(1001+i)),{startCursor:'s10',endCursor:'e10',hasNextPage:false}));await flush();expect(summary.exactCountOrNull).toBe(1100);
});
